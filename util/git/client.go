...
func (m *nativeGitClient) lsRemoteViaFetch(revision string) (string, error) {
	if m.OnLsRemoteViaFetch != nil {
		done := m.OnLsRemoteViaFetch(m.repoURL)
		defer done()
	}
	...
	ctx := context.Background()
	if _, err := tmpClient.runCmd(ctx, "init", "--bare"); err != nil {
	...
	sha, err := tmpClient.runCmd(ctx, "rev-parse", "FETCH_HEAD^{commit}")
	if err != nil {
